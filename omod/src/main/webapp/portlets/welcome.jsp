	...
	<c:when test="${model.authenticatedUser != null}">
		<c:choose>
			<c:when test="${model.showName != 'false'}">
				<spring:message code="welcomeUser" arguments="${model.authenticatedUser.personName.givenName},'Personal Cancer Toolkit'" />
			</c:when>
			<c:otherwise>
				<spring:message code="welcome" arguments="Personal Cancer Toolkit" />
			</c:otherwise>
		</c:choose>
