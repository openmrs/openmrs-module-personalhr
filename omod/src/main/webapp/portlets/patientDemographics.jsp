					...
					<c:forEach var="name" items="${model.patient.names}" varStatus="status">
						<c:if test="${!name.voided}">
							...
							<spring:nestedPath path="name">
								<openmrs:portlet url="../module/personalhr/portlets/phrNameLayout" id="phrNameLayout" size="quickView" parameters="layoutShowExtended=true" />
							</spring:nestedPath>
						</c:if>
					</c:forEach>
				</td>
		...
		<tbody>
			<c:forEach var="address" items="${model.patient.addresses}" varStatus="status">
				...
				<spring:nestedPath path="address">
					<openmrs:portlet url="../module/personalhr/portlets/addressLayout" id="addressPortlet" size="inOneRow" parameters="layoutMode=view|layoutShowTable=false|layoutShowExtended=true" />
				</spring:nestedPath>
				</c:if>
			</c:forEach>
		</tbody>
...
<personalhr:hasPrivilege privilege="Edit Demographics">
<div id="patientDemographicsEdit">

	<openmrs:hasPrivilege privilege="Edit Patients">
		<a href="${pageContext.request.contextPath}/../openmrs/phr/patientForm.form?patientId=${model.patient.patientId}"><spring:message code="Patient.edit"/></a> | 
		<a href="${pageContext.request.contextPath}/phr/newPatient.form?patientId=${model.patient.patientId}"><spring:message code="Patient.edit.short"/></a><br /><br />
	</openmrs:hasPrivilege>
	<openmrs:hasPrivilege privilege="Edit Patients" inverse="true">
		<openmrs:hasPrivilege privilege="Add Patients">
			<a href="${pageContext.request.contextPath}/phr/newPatient.form"><spring:message code="Patient.addNew"/></a><br />
		</openmrs:hasPrivilege>
	</openmrs:hasPrivilege>
</div>
