...
var prefIdentifierElementId = null;
	...
	var relTableCellFuncs = [
		...
		function(data) {
			return '<personalhr:hasPrivilege privilege="Edittable Links">'+'<a href="javascript:editRelationshipDialog(' + data[0] + ')" title="">' +
						'<img src="${pageContext.request.contextPath}/images/edit.gif" border="0" title="<spring:message code="general.edit"/>"/>' +
				   '</a>'+'</personalhr:hasPrivilege>';
		},
		function(data) {
			return '<personalhr:hasPrivilege privilege="Edittable Links">'+'<a href="javascript:voidRelationshipDialog(' + data[0] + ')" title="">' +
				'<img src="${pageContext.request.contextPath}/images/delete.gif" border="0" title="<spring:message code="general.remove"/>"/>' +
				'</a>'+'</personalhr:hasPrivilege>';
		}
	];
			...
			for (var i = 0; i < rels.length; ++i) {
				...
				if (rel.personAId == ${model.personId}) {
					if (rel.personBIsPatient)
						relative = '<%--'+'<personalhr:hasPrivilege privilege="Edittable Links">'+'<a href="patientDashboard.form?patientId=' + rel.personBId + '">'+'</personalhr:hasPrivilege>'+'--%>'
						+ rel.personB + '</a>';
					else
						relative = '<%--'+'<personalhr:hasPrivilege privilege="Edittable Links">'+'<a href="personDashboard.form?personId=' + rel.personBId + '">'+'</personalhr:hasPrivilege>'+'--%>' 
						+ rel.personB + '</a>';
				} else if (rel.personBId == ${model.personId}) {
					if (rel.personAIsPatient)
						relative = '<%--'+'<personalhr:hasPrivilege privilege="Edittable Links">'+'<a href="patientDashboard.form?patientId=' + rel.personAId + '">'+'</personalhr:hasPrivilege>+'--%>'
						+ rel.personA + '</a>';
					else
						relative = '<%--'+'<personalhr:hasPrivilege privilege="Edittable Links">'+'<a href="personDashboard.form?personId=' + rel.personAId + '">'+'</personalhr:hasPrivilege>'+'--%>' 
						+ rel.personA + '</a>';
				}
...
<div id="patientRelationshipPortlet">
	...
	<personalhr:hasPrivilege privilege="Edittable Links">
	...
	<div id="addRelationship">
	 ...
	 <!-- <td>age:<input id="ager" type="text"/></td> -->
	 <td>
		<a id="find_existing_person" href="#">Find an already existing person</a>
	 </td></tr>	
		</table>		
		</div>
	</div>
