	...
	<c:choose>
		...
		<c:otherwise>
			<tr class='${count % 2 == 0 ? "evenRow" : "oddRow"} ${obs.voided ? "voided" : ""}'
				...
				<td class="obsConceptName" style="padding-left: ${padding}"><a href="${pageContext.request.contextPath}/phr/phrObs.form?obsId=${obs.obsId}" onclick="return click('${obs.obsId}')">${obs.concept.name.name}</a></td>
