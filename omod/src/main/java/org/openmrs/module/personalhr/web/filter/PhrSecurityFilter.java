...
public class PhrSecurityFilter implements Filter {
				...
				try {
					...
					if (phrRole != null) {
						...
						if ((requestURI.toLowerCase().contains("index.htm") ||
							...
							if (phrRole != null) {
								...
								getPhrService().logEvent(PhrLogEvent.ACCESS_REDIRECT, new Date(), user, ((HttpServletRequest) request).getSession().getId(), pat,
					...
					if (phrRole != null) {
						if (!requestURI.toLowerCase().contains("/phr/") &&
								...
								!getPhrService().isUrlAllowed(requestURI, pat, per, Context.getAuthenticatedUser())) {

							this.log.debug("***URL access not allowed for this PHR user!!! " + requestURI + "|" + pat + "|" + per + "|"
									+ user);
							getPhrService().logEvent(PhrLogEvent.ACCESS_NOT_ALLOWED, new Date(), user, ((HttpServletRequest) request).getSession().getId(), pat,
					...
					if (enableEventLogging && "POST".equalsIgnoreCase(((HttpServletRequest) request).getMethod())) {
						...
						if (command != null) {
							getPhrService().logEvent(PhrLogEvent.SUBMIT_CHANGES, new Date(), user, ((HttpServletRequest) request).getSession().getId(), pat,
	...
	private PhrService getPhrService() {
		if (phrService == null) {
			phrService = Context.getService(PhrService.class);
		}
		return phrService;
	}
}
