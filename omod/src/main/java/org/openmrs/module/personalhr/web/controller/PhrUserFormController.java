...
public class PhrUserFormController {
    ...
    public String showForm(@RequestParam(required = false, value = "userId") final Integer userId,
        ...
        final PhrSharingToken token = sharingToken == null ? null : Context.getService(PhrSharingTokenService.class)
        ...
        try{
            ...
            if (mss.getMessage("User.assumeIdentity").equals(action)) {
                ...
                if (isNewUser(user) && !isAdministrator) {
                    ...
                    final PhrSharingToken token = Context.getService(PhrSharingTokenService.class).getSharingToken(sharingToken);
