 ...
 */
public class phrUserRegFormController extends SimpleFormController {
	
	/** Logger for this class and subclasses */
	protected static final Log log = LogFactory.getLog(phrUserRegFormController.class);
	...
	protected ModelAndView onSubmit(HttpServletRequest request, HttpServletResponse response, Object obj,
		...
		System.out.println("dodo"+userId+"");
		...
		if (lockedOut) {
			...
			if (secretAnswer == null) {
				...
				if (user == null || user.getSecretQuestion() == null || user.getSecretQuestion().equals("")) {
					...
					//Context.authenticate(user.username, randomPassword);
					httpSession.setAttribute("loginAttempts", 0);
					return new ModelAndView(new RedirectView(request.getContextPath() + "/phr/options.form#Change Login Info"));
				} else {
