...
<script type="text/javascript">
	...
	function doAddAllergy() {
		...
		$j('#allergy_reaction').val("");		$j('#allergy_reaction_id').val("");
		$j('#allergy_type').val("");
	...
	<table style="margin: 0px 0px 1em 2em;" cellpadding="3" cellspacing="0" id="allergyTable" class="alTable">
		...
		<c:forEach var="allergy" items="${model.allergies}">
			...
			<tr bgcolor="${bgColor}">
				...
				<td>
					<c:choose>
						...
						<personalhr:hasPrivilege privilege="Edittable Links">
							<a href="javascript:doEditAllergy(${allergy.activeListId});">
						</personalhr:hasPrivilege>
							<span id="allergen_conceptName_${allergy.activeListId}">
							<openmrs_tag:concept conceptId="${allergy.allergen.conceptId}"/></span>
							</a>
						</c:when>
						<c:otherwise>
				...
				<td>
				<personalhr:hasPrivilege privilege="Edittable Links">
					<a href="javascript:doResolveAllergy(${allergy.activeListId})" title=""><img src="${pageContext.request.contextPath}/images/delete.gif" border="0" title="<spring:message code="ActiveLists.resolve"/>"/></a>
				</personalhr:hasPrivilege>
				</td>
			</tr>
		</c:forEach>
...
<personalhr:hasPrivilege privilege="Add Allergies">
	<div style="width: 100%">
