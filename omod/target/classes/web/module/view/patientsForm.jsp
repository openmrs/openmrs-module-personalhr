...
<openmrs:require privilege="Edit Patients" otherwise="/login.htm" redirect="/phr/phrApply.form"/>
...
<c:if test="${patient.patientId != null}">
	<a href="patientDashboard.form?patientId=${patient.patientId}"><spring:message code="patientDashboard.viewDashboard"/></a>
<%--
	|
				...
				<c:forEach var="identifier" items="${patient.identifiers}" varStatus="varStatus">
					<spring:nestedPath path="patient.identifiers[${varStatus.index}]">
						<div id="identifier${varStatus.index}Data" class="tabBox">
							<%@ include file="editPatientIdentifier.jsp" %>
			...
			<div class="tabBoxes">
				<table>
					<spring:nestedPath path="patient">
						<%@ include file="editPersonInfo.jsp" %>
					</spring:nestedPath>
				</table>
			</div>
		</div>	
